...
| **[jev-kit](./jev-kit)** | Typed decision primitives for a calibrated decision model — it returns a probability, never prose, so there is no JSON to coax out of a chat model. Ships two worked examples with measured numbers and the harness to reproduce them: catching an agent that claims work is done without evidence, and ranking code-review findings by how likely they are to be false positives. Both examples state where the measurement is weaker than it first looked. |
| **[askjev](./askjev)** | A skill for the moment a session reaches a decision nobody is sure about. It puts the live question to the decision model and reports a choice with its full probability spread. Most of the skill is framing discipline rather than API code, because a compound question does not score worse than a split one — it scores backwards, and returns a confident number pointing the wrong way. |
| **[multica-jev](./multica-jev)** | The same decision model wired into an agent platform as a tool the agent can call mid-task: triage, risk, routing and prioritisation, returned as typed answers rather than prose. It has no write path to anything at all, which is the property that makes it safe to hand to an agent: nothing it returns can change anything on its own. A pattern list additionally refuses the obvious phrasings of judgments that should stay with a human, and the README is explicit that this catches wording rather than meaning. Ships adapters for OpenCode and Pi off one shared core. |
...
| **Bun, not npm or node** | Every local tool is a Bun script or plain POSIX shell — nothing needs node. **Bun ≥ 1.3** specifically — config parsing uses the built-in `Bun.YAML`, which is what lets the skills ship with zero runtime dependencies and run the moment you copy them. **One exception, stated rather than buried:** `multica-jev` is an npm package for a Node-based agent host, so it needs Node ≥ 20 and an `npm install`. Nothing else here does, and nothing else depends on it. |
