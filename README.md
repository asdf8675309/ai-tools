...
| **[runaway](./runaway)** | The failure the hooks cannot see: memory eaten until the Mac puts up "your system has run out of application memory" and the only option left is force-quit. A LaunchAgent that reads the kernel's own pressure signal, the compressor, and your swapout rate, and **pauses** the largest process it is allowed to touch — SIGSTOP, nothing lost, undone automatically if pressure clears and escalated to a kill if it does not. Plus a per-process cap and a disk floor. Built for what a coding agent spawns; not limited to it, because the thing filling your swap usually is not. |
...
| **macOS, mostly by default rather than by requirement** | This was built and used on macOS. Almost everything is POSIX shell and TypeScript that runs anywhere. Two genuine exceptions, both stated where they live. **`runaway` is macOS-only by requirement** — it reads swap through `sysctl vm.swapusage` and installs itself as a LaunchAgent; its installer refuses to run anywhere else, though its decision logic is portable and its tests run on Linux in this repo's CI. The other is two spots in the optional Metis integration: starting Docker Desktop (`open -a`) and quitting it (`osascript`), both of which degrade to a clear "Docker is down, skipping" elsewhere. |
...
Nearly all of the code and prose in all four tools was drafted with Claude Code, directed and reviewed by me. Saying so up front rather than leaving you to infer it: tools that ask you to trust their judgment about your code should be forthright about their own provenance.
