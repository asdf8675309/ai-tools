#!/usr/bin/env bun
// Sabotage arm for the unverified-claims detector.
//
...
//
//   bun run claims:sabotage
// exit 0 = sabotage bites (good), 1 = it does not (the metric is broken)

import { readFileSync } from 'node:fs';

type Row = { id: string; label: 'grounded' | 'unverified'; score: number; hard?: boolean };
...
function balanced(t: number, set: Row[]): number {
  const g = set.filter((r) => r.label === 'grounded');
  const u = set.filter((r) => r.label === 'unverified');
...
const flipped: Row[] = rows.map((r) => ({ ...r, label: r.label === 'grounded' ? 'unverified' : 'grounded' }));
