...
| **[runaway](./runaway)** | The failure the hooks cannot see: a process the agent spawned eating memory until the Mac is unusable. A LaunchAgent watching the process tree under your agent, with a per-process cap, a swap ceiling, and a disk floor — built to fire early enough that killing the process still works, and scoped so it can only ever signal something you own and your agent started. |
