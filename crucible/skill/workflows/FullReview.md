...
**Forcing a single model for an eval run:** rather than a CLI flag, override `models.reviewer_*` in a temporary `.crucible.yaml` overlay, or — when running the journaled dynamic-workflow edition (`WorkflowMode.md`) — pass an `enumerationModel` workflow arg that the script applies uniformly across reviewers. Two limits on the overlay route. First, **the overlay must be untracked** — leave it out of the index, and delete it when the run ends. `models` is clamped out of any overlay the reviewed repo tracks, so committing it produces a run against the skill defaults, not the model you named; `loadConfig()` says so on stderr rather than failing silently. Second, it reaches only keys that already resolve: the `gateway_model_map` / `local_model_map` / `external_cli_map` entry a `gateway-`/`local-`/`cli-` key needs, and the `integrations.*.enabled: true` behind it, are clamped from a working-tree overlay however it arrived, and would leave the slot falling down `reviewer_fallback_chain` to Claude — which is not the model you meant to evaluate. Point an untracked overlay at a `claude-*` key, or edit the skill's own `config.yaml` for the duration of the run.
