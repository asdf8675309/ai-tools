...
**Configuration.** `config.yaml` plus `tools/Config.ts` — one source of truth for model slots, thresholds, integrations, and flags. A `.crucible.yaml` at any repo root deep-merges over it, so per-project tuning never requires forking the skill — clamped, because an overlay lives in the reviewed working tree: it cannot enable an integration or name a command, endpoint, or credential env var (`OVERLAY_PROTECTED_PATHS` in `tools/Config.ts`). It *can* still retune thresholds and flags, so an overlay in a diff is part of what needs reviewing. `models` and `degraded_overrides` are clamped as well whenever git tracks the overlay in the repo under review — a `.crucible.yaml` that arrives with a PR cannot choose which model reviews it.
