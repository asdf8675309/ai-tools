...
| **[multica-jev](./multica-jev)** | The same decision model wired into an agent platform as a tool the agent can call mid-task: triage, risk, routing and prioritisation, returned as typed answers rather than prose. It has no write path to anything, and it refuses outright to make the judgments that should stay with a human — resolving a review thread, ruling a finding real, scoping a card. Ships adapters for OpenCode and Pi off one shared core. |
