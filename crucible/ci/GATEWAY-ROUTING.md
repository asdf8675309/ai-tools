...
- **A budget or degraded-mode override that re-points a slot outside the routing layer throws away the model choices you measured.** This is worth stating because it is a natural thing to build and it quietly undoes the work. If a cost-pressure rule swaps a reviewer onto some cheaper path that bypasses the gateway, that lens is no longer running the model your evaluation chose, and the band you happened to be in becomes an input to which model judged your code. Two ways out. Either express the cheap option as another route and keep every path inside the same layer, or leave the override off.

  The repository now ships a scoped version of this, `degraded_overrides`, and the warning above is exactly why it is shaped the way it is. It is **off unless a caller passes a band** — nothing here discovers one, so a default checkout can never enter a degraded band by accident. It is clamped like `models` for a git-tracked overlay, so a PR cannot pick its own degraded reviewers. And the preferred configuration is still the first way out: point the override at **another gateway route**, not at a model outside the routing layer, so the lens keeps running a model your evaluation chose. Re-pointing to a non-gateway key works and is sometimes what you want under real cost pressure, but it discards the measurement, and that is a decision to make deliberately rather than a default to inherit.
